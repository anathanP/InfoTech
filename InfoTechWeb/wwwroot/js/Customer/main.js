...
(function($) {
    ...
    $(document).ready(function() {
        ...
        function followImageCursor(event, teamItems, isRTL) {
            const contentBox = teamItems.getBoundingClientRect();
            let dx;
            if (isRTL) {
                dx = event.clientX - contentBox.right;
            } else {
                dx = event.clientX - contentBox.left;
            }
        ...
        teamItems.forEach((item, i) => {
            let isRTL = window.getComputedStyle(item).direction === "rtl";
            item.addEventListener("mousemove", (event) => {
                followImageCursor(event, item, isRTL);
            });
        });
